# -*- coding: utf-8 -*-
"""
네이버뉴스 경제 > 금융 섹션 -> finance.txt 로 저장 (GitHub Actions용)
------------------------------------------------------------------------
1. 네이버뉴스 경제>금융 섹션 페이지(HTML)를 직접 읽어옴
2. 기사 제목 + 링크를 최대 10개까지 뽑음
3. finance.txt / finance-{요일}.txt 파일로 저장 (커밋/푸시는 워크플로우 쪽에서 처리)
naver_realestate_fetch.py / world_news_fetch.py 와 같은 방식.
"""
...
SECTION_URL = "https://news.naver.com/breakingnews/section/101/259"  # 경제 > 금융
MAX_ARTICLES = 10
...
def fetch_finance_headlines(url, max_articles=10):
...
def main():
    print("금융 뉴스를 가져오는 중...")
    articles = fetch_finance_headlines(SECTION_URL, MAX_ARTICLES)
    ...
    headline = f"🏦 {now.strftime('%y')}년 {now.month}월 {now.day}일 {weekday_kr} 금융 뉴스"
